 ...
 */
export async function createPrompt(app: App, folder: string, draft: PromptDraft, typeKey: string): Promise<TFile> {
	...
	await app.fileManager.processFrontMatter(file, (fm: Record<string, unknown>) => {
		Object.assign(fm, draftToFrontmatter(draft, typeKey));
		fm["created"] = today;
		fm["updated"] = today;
	});
	return file;
 ...
 */
export async function updatePrompt(app: App, file: TFile, draft: PromptDraft, typeKey: string): Promise<void> {
	await app.fileManager.processFrontMatter(file, (fm: Record<string, unknown>) => {
